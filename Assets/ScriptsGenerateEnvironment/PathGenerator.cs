...
public class PathGenerator : MonoBehaviour
{
    ...
    public void GenerateLandmarks(Mesh[] meshes)
    {
        ...
        {    
            ...
            {
                Vector3 vertice = landMarkCoords[j] + Vector3.down * 0.5f;
                ...
                vertice -= CalibrateHeight(vertice + center, extents);
                
                if (IsNotOverlappingPath(vertice + center, extents))
                {
                    // Add a cube beneath a landmark to prevent floating objects
                    GameObject cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
                    cube.transform.localScale = extents * 2;
                    cube.transform.position = vertice + center + Vector3.down * (extents.y * 2f + 0.02f);
    ...
    public Vector3 CalibrateHeight(Vector3 vertice, Vector3 extents)
    {
        Collider[] cols = Physics.OverlapBox(vertice, extents);
    ...
    public bool IsNotOverlappingPath(Vector3 vertice, Vector3 extents)
    {
        Collider[] cols = Physics.OverlapBox(vertice, extents);
        for (int i = 0; i < cols.Length; i++)
        {
            // These may overlap
            if (cols[i].name == "PathMesh")
            {
                return false;
            }
        }
    ...
    {
        GameObject gameObjectToSpawn = new GameObject("PathMesh");

        gameObjectToSpawn.AddComponent<MeshFilter>();
        gameObjectToSpawn.AddComponent<MeshCollider>();
        gameObjectToSpawn.AddComponent<MeshRenderer>();
        gameObjectToSpawn.GetComponent<MeshRenderer>().material = mat;
        gameObjectToSpawn.GetComponent<MeshFilter>().mesh = mesh;
        gameObjectToSpawn.GetComponent<MeshCollider>().sharedMesh = mesh;
        gameObjectToSpawn.GetComponent<MeshCollider>().enabled = true;

        gameObjectToSpawn.layer = LayerMask.NameToLayer(layer);
    }
